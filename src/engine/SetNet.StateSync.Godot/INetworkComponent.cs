...
{
    /// <summary>
    /// A node that contributes replicated fields to a <see cref="NetworkObject"/>. An object's archetype schema is the
    /// ordered concatenation of its network components' fields, so the child order in the scene must be identical on the
    /// server and every client (it is — they instantiate the same scene). Implement <see cref="DeclareFields"/> to
    /// describe the fields, then read/write them in the same order in <see cref="Serialize"/> (server → wire) and
    ...
    public interface INetworkComponent
    {
        ...
        /// <summary>Server side: reads the current Godot state and writes it into the entity via the cursor.</summary>
        void Serialize(NetworkWriter writer);

        /// <summary>Client side: reads the interpolated entity state from the cursor and applies it to Godot.</summary>
