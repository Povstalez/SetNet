...
- **WebSocket transport (optional, separate package `SetNet.WebSockets`; depends only on `SetNet`)**: runs SetNet over `ws://` instead of raw TCP/UDP. `config.UseWebSockets()` sets `TransportType.Custom` + a `WebSocketTransport : ITransportProvider`; everything above the transport (handlers, RPC, rooms, auth) is unchanged. Client via `System.Net.WebSockets.ClientWebSocket`, server via `HttpListener.AcceptWebSocketAsync`. Framing is one binary WS message per SetNet frame: `[2-byte type LE][payload]` (WS message boundaries replace TCP's length prefix). Reliable/ordered like TCP, so `DeliveryMethod` is ignored. `wss://` via a TLS-terminating reverse proxy. **Unity + WebGL variant: `SetNet.WebSockets.Unity`** (UPM source package, `src/engine/SetNet.WebSockets.Unity/`, NOT in the sln — Unity-only, uses `#if UNITY_WEBGL`/`[DllImport("__Internal")]`): a **client** WebSocket transport that also works in **Unity WebGL** (where `System.Net.WebSockets` is unavailable). `config.UseUnityWebSockets(secure)` → `UnityWebSocketTransport : ITransportProvider` (client-only; `CreateListener` throws). Off WebGL it uses `ClientWebSocket` (`SystemWebSocketConnection`); on WebGL a `.jslib` bridge (`Plugins/WebGL/SetNetWebSocket.jslib`, polling — queues incoming ArrayBuffers, no dynCall) + `WebGLWebSocketConnection` (`ReceiveAsync` polls, `await Task.Yield()` to the single-threaded player loop). Same `[2-byte type LE][payload]` framing, so one `SetNet.WebSockets` server serves WebGL + native. Drive on the main thread; pair with `SetNet.Unity.MainThreadDispatcher`; `wss://` for https/Telegram. (Can't be compiled/tested outside a Unity build.)
...
- **Hitscan / Projectiles — shooting with a pluggable hit test (optional, separate packages; server-side, no wire)**: the hit test is the seam **`IHitDetector.Raycast(in Ray, in HitQuery) → HitResult`** — the app implements it (own collision), or composes shipped detectors. `Ray`(origin+normalized dir), `HitQuery`(shooterId/faction/maxDistance/context), `HitResult`(Hit/Kind{Target,World}/Point/Distance/Normal/TargetId/Target). **`SetNet.Hitscan`** (deps `SetNet.GeoData`): `TargetHitDetector` (ray-vs-sphere over `ITargetProvider`'s `HitTarget`s, skips shooter), `GeoDataHitDetector` (`IGeoData.Raycast`), `CompositeHitDetector` (closest of several — world blocks target behind), `HitscanResolver.Fire(origin,dir,shooterId,maxDistance?,faction?,context?)`→`HitResult` + `OnShot`/`OnHit`. (NB: main class is `HitscanResolver`, not `Hitscan`, to avoid clashing with the namespace.) **`SetNet.Projectiles`** (deps `SetNet.Hitscan`+`SetNet.GeoData`+`SetNet.Ticks`): `ProjectileSystem` advances projectiles and **sweep-tests** each step's segment through the SAME `IHitDetector` (no tunnelling); `ProjectileOptions.Gravity` (ballistics), lifetime/range, `Spawn`/`Hit`/`Expired` events; implements `ITickable` → auto-subscribes to `TickHost.Current` (channel "projectiles" @Hz) or `Update(dtMs)`/internal timer. Replicates nothing.

- **Inventory.Grid — spatial "tetris" inventory (optional, separate package `SetNet.Inventory.Grid`; ZERO deps, pure data)**: `GridInventory(width,height)` with `GridItem`(Type/Width/Height/Count/MaxStack/Tag), `GridPos`, `GridRotation`(None/Quarter=90°), `PlacedItem`(effective Width/Height by rotation). `TryPlaceAt(item,pos,rot)`/`TryAdd(item)` (auto-fit, tries both rotations)/`TryMove`/`TryRotate`/`Remove`; `CanPlace`/`IsFree`/`At(pos)`/`Get(id)`/`FreeCells`; optional stacking (merge same `Type` up to `MaxStack`, **all-or-nothing** with rollback), `Changed` event. Complements the quantity-based `SetNet.Inventory`.
...
- **Additional companion packages (see [docs/MODULES.md](docs/MODULES.md) for the full catalog)** — all composition, same patterns. Serializers via `ISerializer`: `SetNet.Json`, `SetNet.MemoryPack`, `SetNet.Protobuf`, `SetNet.Compression` (Brotli decorator). Security: `SetNet.Auth.Jwt`/`SetNet.Auth.OAuth` (`IAuthenticator`s), `SetNet.BanList` (ban gate via `InboundAuthorizer` + kick, `IBanStore`), `SetNet.DdosGuard` (per-IP flood auto-ban on `SetNet.BanList`). Net/QoS: `SetNet.Fragmentation` (oversize-UDP split/reassemble; reserved 65517; uses `InjectFrame`), `SetNet.Priority` (`PrioritySender`), `SetNet.Congestion` (AIMD `CongestionController`), `SetNet.Ping` (RTT latency: `server.UsePing().Of(peer)`→ms any time + `client.UsePing()` echo/measure; rides `Channels.Ping`=34; monotonic Stopwatch timestamp round-trips on the initiator's clock so no sync needed; both ends call `UsePing()`, `PingRuntime.Enable()`). Realtime: `SetNet.Rooms.HostMigration` (uses `RoomServer.RoomHooks()`), `SetNet.Party`, `SetNet.Chat`, `SetNet.Lockstep` (all four now ride the unified `SetNet.Protocol` on their `Channels` id — see the Unified messaging protocol bullet), and StateSync sub-modules `SetNet.StateSync.{SpatialGrid,LagCompensation,Prediction,NetworkVariable,Rpc}` (Rpc reserved 65516). Infra: `SetNet.DependencyInjection` (`provider.UseSetNet()` routes construction of **all** reflection-discovered components — `[MessageHandler]`, `[ProtocolChannel]` channel services/`[Op]` classes, client `[Event]` handlers, `[RpcMethod]` RPC handlers — through the one core `HandlerActivator.Factory` seam, with constructor injection via `ActivatorUtilities`; `UseSetNetHandlers()` kept as alias), `SetNet.Services` (tiny service **locator** — `ServiceHub`: `hub.Add(server.UseXxx())` once, resolve anywhere by type via ambient `Service.Get<T>()` (`MakeCurrent`) or per-owner `server.Services()`/`client.Services()` (`ConditionalWeakTable`); locates the module singletons, constructs nothing; deps core only), `SetNet.Hosting` (`IHostedService`), `SetNet.HealthChecks` (`IHealthCheck`), `SetNet.Inspector` (HttpListener `/metrics`+HTML dashboard), `SetNet.Gateway` (raw-relay reverse proxy via `OnRawFrame`/`SendRawAsync`). Engine: `SetNet.Godot` (Godot 4 dispatcher + math, deps `GodotSharp`).
