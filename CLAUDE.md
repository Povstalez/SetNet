...
- **Core hooks added for these (all general-purpose, additive)**: `PeerInfo.RemoteEndPoint`+`BasePeer.RemoteEndPoint` (ban/DDoS/inspector/gateway), `BaseSocket.InjectFrame(type,data)` (fragmentation reassembly / relays), `HandlerActivator.Factory` (DI handler construction), `RoomServer.RoomHooks()` (Rooms server-side join/left events for host migration). Reserved wire-type map is now contiguous **65448–65535** (see MODULES.md).
