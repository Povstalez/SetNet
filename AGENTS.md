...
- **Custom transport hook (core)**: `TransportType.Custom` + `Configuration.CustomTransport : ITransportProvider`, resolved in `TransportFactory` — lets external packages add a transport without editing core (used by `SetNet.WebSockets`).
