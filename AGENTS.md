...
- **RPC (optional, separate package `SetNet.Rpc`; depends only on `SetNet`, serializer-agnostic)**: request/response by **composition**, no base class. `await client.CallAsync<TReq,TResp>(methodId, req)` (extension) + `[RpcMethod]` `IRpcHandler<TReq,TResp>` server handlers. The envelope is hand-framed as a `byte[]` (rides over any serializer, no MessagePack dependency); bodies go through `SetNetSerializer`. It has **no wire ids of its own**: `CallAsync(methodId, req)` is a thin alias for `RequestAsync(Channels.Rpc, methodId, req)`, so it rides the unified protocol envelope (65447) and its correlation registry. Server side, one `[ProtocolChannel(Channels.Rpc)] RpcChannelService` routes each request to the discovered `[RpcMethod] IRpcHandler<TReq,TResp>`; a throwing handler or unknown method surfaces as `RpcException`, and per-call timeouts come from `RequestAsync`. Call `RpcRuntime.Enable()` once at startup so the package is loaded before handler discovery.
...
- **Rooms/Lobbies (optional, separate package `SetNet.Rooms`; depends only on `SetNet`, serializer-agnostic)**: rooms by **composition** on a dedicated server (server is the hub — no relay). `server.UseRooms(IRoomStore?)` + `client.UseRooms()` → `CreateAsync`/`JoinAsync(code)`/`LeaveAsync`/`BroadcastAsync<T>` + `PlayerJoined`/`PlayerLeft`/`MessageReceived`/`Closed`. Reserved types 65526/65527/65528; pluggable `IRoomStore` (default `MemoryRoomStore`). Uses the core `BaseServer.PeerDisconnected` event (added for it, general-purpose) to auto-remove a dropped peer and notify others. Call `RoomsRuntime.Enable()` at startup.

- **WebSocket transport (`SetNet.WebSockets`, deps `SetNet` only)**: `config.UseWebSockets()` → `TransportType.Custom` + `WebSocketTransport : ITransportProvider`; runs over `ws://` (client `ClientWebSocket`, server `HttpListener`), one binary WS message per frame (`[2-byte type LE][payload]`), reliable/ordered like TCP. Everything above the transport unchanged.
- **In-memory transport (`SetNet.InMemory`, deps `SetNet` only)**: `config.UseInMemory()` → `TransportType.Custom` + `InMemoryTransport`; client+server sharing a `Host:Port` key connect in-process (no sockets) via linked `InMemoryConnection`s over an `AsyncChannel<T>`, reliable/ordered. Server must `Start()` before client connects. For fast deterministic tests + co-hosting.
- **Matchmaking (`SetNet.Matchmaking`, deps `SetNet` + `SetNet.Rooms`)**: `server.UseMatchmaking(store, MatchmakingOptions)` (same store as `UseRooms`) + `client.UseMatchmaking().FindMatchAsync(MatchRequest)`. Background `Timer` forms per-queue matches (FIFO or skill-based with a widening acceptance window); creates a room in the shared store and pushes targeted `MatchFound{code}`; clients then `rooms.JoinAsync(code)`. Reserved 65523/65524/65525; uses `PeerDisconnected` to drop leavers. Node-local.
- **Rate limiting (`SetNet.RateLimit`, deps `SetNet` only)**: `server.UseRateLimit(RateLimitOptions{PerPeerPerSecond,Burst})` → token-bucket gate chained onto `BaseServer.InboundAuthorizer`, per-peer bucket in a `ConditionalWeakTable`; over-budget frames dropped. Composes with Auth's gate.
- **Unity (`SetNet.Unity`, deps `SetNet` only)**: `MainThreadDispatcher` (`Post`/`PostAsync`/`Drain`) — marshal handler callbacks onto the Unity main thread; drain in `Update()`. WebGL unsupported.
- **Serilog / NLog / ZLogger (`SetNet.Logging.{Serilog,NLog,ZLogger}`, deps `SetNet` + the logger)**: `config.Logger = new {Serilog,NLog,ZLogger}Logger(...)` maps `ILogger.Log(msg, LogLevel)` into that backend. Serilog defaults to `global::Serilog.Log.Logger`; NLog defaults to `LogManager.GetLogger("SetNet")`; ZLogger wraps a `Microsoft.Extensions.Logging.ILogger` (csproj `LangVersion=latest` for `ZLog*` interpolated handlers). Adapters live in namespace `SetNet.Logging`, `global::`-qualified to avoid shadowing.
- **Custom transport hook (core)**: `TransportType.Custom` + `Configuration.CustomTransport : ITransportProvider`, resolved in `TransportFactory` — lets external packages add a transport without editing core (used by `SetNet.WebSockets`, `SetNet.InMemory`).
- **State replication (`SetNet.StateSync`, deps `SetNet` only)**: server-authoritative entity replication. `server.UseStateSync(opts)` → `ServerReplication` (Spawn/Despawn/AddObserver/InputReceived); `client.UseStateSync()` → `ClientReplication` (EntitySpawned/EntityDespawned, Entities, OwnedEntity, SendInput, Update). Archetype field schemas via `ReplicaRegistry`/`ReplicaSchema` (identical both ends); framework interpolates float/vector/quat, snaps discrete, optional float quantization. Fixed-rate Timer sends delta-compressed snapshots (tick/baseline-ack history) over Unreliable; Spawn/Despawn Reliable. Pluggable `IInterestManager` (`AllInterest`/`DistanceInterest`). Input channel = prediction plumbing. Reserved 65518–65522. Uses the new **`BaseServer.PeerConnected`** core hook (symmetric with PeerDisconnected) for auto-observe. `StateSyncRuntime.Enable()`.
- **Unity binding (`SetNet.StateSync.Unity`, UPM source, references UnityEngine — not NuGet)**: `NetworkObject` + `NetworkTransform`/`NetworkAnimator`(params+triggers+layer state)/`NetworkRigidbody`/`NetworkBehaviour` (each an `INetworkComponent` contributing fields) + `NetworkManager` (prefab registry, spawn/despawn, drives tick+interp, marshals via `SetNet.Unity.MainThreadDispatcher`). Object schema = ordered concat of its components' fields.

- **More companion packages (full catalog: [docs/MODULES.md](docs/MODULES.md))** — all composition. Serializers (`ISerializer`): `SetNet.Json`, `SetNet.MemoryPack`, `SetNet.Protobuf`, `SetNet.Compression`. Security: `SetNet.Auth.Jwt`, `SetNet.Auth.OAuth`, `SetNet.BanList`, `SetNet.DdosGuard`, `SetNet.GeoBlock` (kick-by-country on `PeerConnected`, pluggable `IGeoResolver`), `SetNet.ProofOfWork` (hashcash gate 65506/65507, chains `InboundAuthorizer`). Net/QoS: `SetNet.Fragmentation` (reserved 65517, uses `InjectFrame`), `SetNet.Priority`, `SetNet.Congestion`, `SetNet.Relay` (TURN-style opaque relay hub 65498-65500; Command/Reply/Event like Rooms — server replies via `SendAsync`, not `SendRawAsync`). Realtime: `SetNet.Rooms.HostMigration` (65508, uses `RoomServer.RoomHooks()`), `SetNet.Party` (65511-65513), `SetNet.Chat` (65509/65510), `SetNet.Voice` (codec-agnostic relay 65503-65505, unreliable), `SetNet.Lockstep` (65514/65515), `SetNet.StateSync.{SpatialGrid,LagCompensation,Prediction,NetworkVariable,Rpc}` (Rpc 65516). Infra: `SetNet.DependencyInjection` (`HandlerActivator.Factory`), `SetNet.Hosting`, `SetNet.HealthChecks`, `SetNet.Inspector`, `SetNet.Gateway` (raw relay), `SetNet.Cluster` (server-to-server broadcast mesh 65501, own listener/`OnRawFrame`), `SetNet.Redis` (Redis backplane: shared `ISessionStore`/`IBanStore`/`IRoomStore`, deps Auth+Rooms+BanList+StackExchange.Redis, no wire types). Engine: `SetNet.Godot`. Reserved wire-type block now spans **65498–65535**.
- **Core hooks for these (general-purpose)**: `PeerInfo.RemoteEndPoint`/`BasePeer.RemoteEndPoint`, `BaseSocket.InjectFrame`, `HandlerActivator.Factory`, `RoomServer.RoomHooks()`. Reserved wire types now contiguous **65508–65535**.
...
Only a local `Disconnect()`/`Dispose()` is intentional; a remote-initiated close is a loss (opt out with `ReconnectOnRemoteClose = false`). On UDP/Both/WebSocket an orderly end-of-stream is the only shape a drop takes, since their receive path never throws. Each attempt owns a **generation** id, so a receive loop unwinding from a replaced connection cannot tear down the live one.
...
Server-side, `BasePeer` mirrors this: `Close()` (kick) → only `OnDisconnected`; client crash/IO error or heartbeat timeout → `OnError` + `OnUnexpectedDisconnect` + `OnDisconnected`. Peer liveness is refreshed by **any** inbound frame, not just Pings, so an active client is never reaped over a heartbeat-config mismatch.
