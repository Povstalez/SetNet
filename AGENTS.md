...
- **Serilog (`SetNet.Logging.Serilog`, deps `SetNet` + `Serilog`)**: `config.Logger = new SerilogLogger(logger?)` maps `ILogger.Log(msg, LogLevel)` into Serilog (defaults to `global::Serilog.Log.Logger`).
...
- **More companion packages (full catalog: [docs/MODULES.md](docs/MODULES.md))** — all composition. Serializers (`ISerializer`): `SetNet.Json`, `SetNet.MemoryPack`, `SetNet.Protobuf`, `SetNet.Compression`. Security: `SetNet.Auth.Jwt`, `SetNet.Auth.OAuth`, `SetNet.BanList`, `SetNet.DdosGuard`. Net/QoS: `SetNet.Fragmentation` (reserved 65517, uses `InjectFrame`), `SetNet.Priority`, `SetNet.Congestion`. Realtime: `SetNet.Rooms.HostMigration` (65508, uses `RoomServer.RoomHooks()`), `SetNet.Party` (65511-65513), `SetNet.Chat` (65509/65510), `SetNet.Lockstep` (65514/65515), `SetNet.StateSync.{SpatialGrid,LagCompensation,Prediction,NetworkVariable,Rpc}` (Rpc 65516). Infra: `SetNet.DependencyInjection` (`HandlerActivator.Factory`), `SetNet.Hosting`, `SetNet.HealthChecks`, `SetNet.Inspector`, `SetNet.Gateway` (raw relay). Engine: `SetNet.Godot`.
